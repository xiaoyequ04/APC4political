...
wvs_china <- WVS_TimeSeries_1981_2022_spss_v3_0 %>%
  filter(.,COW_NUM == 710) %>%
  select(.,S002VS,S008,S020,COW_NUM,COW_ALPHA,E069_11,E069_12,E069_08,E069_07,E114,E115,E116,E117,E224,E225,E226,E227,E228,E229,E235,E236,E035,E036,E037,E039,X001,X002,X003,X025,X025R,X045,X047_WVS,X047R_WVS,X007,S017,E023,G006,X052,X051,D059,D060,D078,C001,F118,F120,F121,F122,F123,D057,A173,G007_18_B,G007_33_B,G007_34_B,G007_35_B,G007_36_B,E018) %>%
  dplyr::rename(
    wave = S002VS,
    period = S020,
    province = S008,
...
## job
wvs_china$equali_gender <- wvs_china %>%
  select(.,D078_na,D060_na) %>%
  rowMeans(na.rm = TRUE)
wvs_china$equali_genderz <- scales::rescale(wvs_china$equali_gender,to = c(0,1))

## tolerace
# wvs_china$tolerance <- wvs_china %>%
#   select(.,F118_na,F120_na,F121_na,F122_na,F123_na) %>%
#   rowMeans(na.rm = TRUE)

wvs_china$tolerancez <-scales::rescale(wvs_china$F118_na,to = c(0,1))
## bure
wvs_china$E036_na <- 11 - wvs_china$E036_na
wvs_china$private <- wvs_china$E036_na # 国营经济成分应该缩小
wvs_china$privatez <-scales::rescale(wvs_china$private,to = c(0,1))
wvs_china$compete <- 11 - wvs_china$E039_na #竞争
wvs_china$competez <-scales::rescale(wvs_china$compete,to = c(0,1))
wvs_china$individual <- wvs_china$E037_na # 个人应该承担更多的责任
wvs_china$individualz <-scales::rescale(wvs_china$individual,to = c(0,1))


## repect
wvs_china$respect <- scales::rescale(wvs_china$E018_na,to = c(0,1))
## social
wvs_china$G007_18_B_na <- 5 - wvs_china$G007_18_B_na
wvs_china$G007_33_B_na <- 5 - wvs_china$G007_33_B_na
wvs_china$G007_34_B_na <- 5 - wvs_china$G007_34_B_na
wvs_china$G007_35_B_na <- 5 -  wvs_china$G007_35_B_na
wvs_china$G007_36_B_na <- 5 -  wvs_china$G007_36_B_na
wvs_china$trustsocial <-  wvs_china %>%
  select(.,G007_18_B_na,G007_33_B_na,G007_34_B_na) %>%
  rowMeans(na.rm = TRUE)
wvs_china$trustsocialz <-scales::rescale(wvs_china$trustsocial,to = c(0,1))
...
wvs_china$E228r_na <- Recode(wvs_china$E228_na,"1:5 = 1; 6:10 = 0")
wvs_china$E229r_na <- Recode(wvs_china$E229_na,"1:5 = 0; 6:10 = 1")
wvs_china$E224s_na <- 11 - wvs_china$E224_na
wvs_china$E227s_na <- 11 - wvs_china$E227_na
wvs_china$E228s_na <- 11 - wvs_china$E228_na
wvs_china$demscore_sub <- wvs_china %>%
  select(.,E224s_na,E226_na,E227s_na,E229_na) %>%
  rowMeans(na.rm = TRUE)
...
wvs_china$demimportantz <- scales::rescale(as.numeric(wvs_china$demimportant),to =c(0,1))
...
wvs_china <- wvs_china %>%
  mutate(
    ...
    cohort_political = case_when(
      yearbirth %in% c(1905:1931) ~ "Pre",
      yearbirth %in% c(1931:1948) ~ "Socialist",
      yearbirth %in% c(1948:1960) ~ "Cultural Revolution",
...
wvs_china_group <- wvs_china %>%
  ...
  filter(.,cohort != "NA") %>%
  gather(.,key = "demconcept",
         value = "score",
         - "cohort")

p1_pro <- wvs_china_group %>%
  ggplot(aes(x = cohort, y = score,group = demconcept)) +
  geom_line(aes(linetype = demconcept,color = demconcept)) +
  geom_point(aes(color = demconcept)) +
  scale_y_continuous(limits = c(7, 10)) +
  scale_linetype_manual(values=c("twodash", "twodash"))+
  scale_color_manual(values=c('#999999','#E69F00'))+
  ggtitle("The Concept of Democracy") +
  xlab("Cohort") +
  ylab("Demscores")
...
fm1_cohort_pro <- broom::tidy(fm_pro) %>%
  ...
  rename(est_pro = estimate,
         se_pro = std.error,
         cohort = term) %>%
  mutate(upper_pro = est_pro + 1.96*se_pro,
         lower_pro = est_pro - 1.96*se_pro)


fm1_cohort_uti <- broom::tidy(fm_uti) %>%
  filter(grepl("^co",term)) %>%
  rename(est_uti = estimate,
         se_uti = std.error,
         cohort = term) %>%
  mutate(upper_uti = est_uti + 1.96*se_uti,
         lower_uti = est_uti - 1.96*se_uti)
...
pf_cohort_pro <- ggplot(fm1_cohort_pro, aes(x = cohort, y = est_pro)) +
  geom_point() + 
  geom_errorbar(aes(ymin = lower_pro, ymax = upper_pro), width = .15) + 
...
pf_cohort_uti <- ggplot(fm1_cohort_uti, aes(x = cohort, y = est_uti)) +
  geom_point() + 
  geom_errorbar(aes(ymin = lower_uti, ymax = upper_uti), width = .15) + 
  scale_y_continuous(limits = c(-2,2)) +
  geom_hline(
    yintercept = 0,
    colour = "grey60",
...
pf_cohort_pro2uti_wvs

fm_diff <- update(fm_pro,demscore_sub ~ .)
fm_demscore <- update(fm_pro,demscore ~ .)

fm1_cohort_diff <- broom::tidy(fm_diff) %>%
  filter(grepl("^co",term)) %>%
  rename(est = estimate,
         se = std.error,
         cohort = term) %>%
  mutate(upper = est + 1.96*se,
         lower = est - 1.96*se)
pf_cohort_diff <- ggplot(fm1_cohort_diff, aes(x = cohort, y = est)) +
  geom_point() + 
...
fm1_cohort_demscore <- broom::tidy(fm_demscore) %>%
  filter(grepl("^co",term)) %>%
  rename(est = estimate,
         se = std.error,
         cohort = term) %>%
...
pf_cohort_demscore <- ggplot(fm1_cohort_demscore, aes(x = cohort, y = est)) +
  geom_point() + 
  geom_errorbar(aes(ymin = lower, ymax = upper), width = .15) + 
  # scale_y_continuous(limits = c(-2,2)) +
  geom_hline(
...
lm_concept <- lmerTest::lmer(demscore ~ age + female + party + edu + interstpol + frepol + socialLevel + hukouRural + marriage  + (1|cohort_political) + (1|year),data = cbs,weights = weight)
...
```{r}
cbs$nationalism <- cbs %>%
  select(.,lifestyle,systemsuit) %>%
  rowMeans(na.rm = TRUE)
fm_nationalism <- update(fm_autho,nationalism ~.)
fm1_cohort_nationalism <- broom::tidy(fm_nationalism) %>%
  filter(grepl("^as.",term)) %>%
  rename(
    est = estimate,
    se = std.error,
...
fm1_cohort_nationalism$cohort <- gsub("^(as.factor)", "",fm1_cohort_nationalism$cohort)

pf_cohort_nationalism <- ggplot(fm1_cohort_nationalism, aes(x = cohort, y = est)) +
  geom_point() + 
  geom_errorbar(aes(ymin = lower, ymax = upper), width = .15) + 
  scale_y_continuous(limits = c(-.8,.8)) +
  geom_hline(
    ...
    linetype = 2) +
  xlab("Cohort") + 
  ylab("The cohort effect of Nationalism")
pf_cohort_nationalism
```
...
fulldata_wvs <- wvs_china %>%
  ...
  ungroup() %>%
  filter(.,yearbirth >= "1950" & yearbirth < "2002")

fulldata_wvs$gdppc_mean_log <- log(fulldata_wvs$gdppc_mean)


## 
gdppc2demscore <- lm(demscore ~ gdppc_mean_log  + age + gender + edu  + interstpol + marriage + trustsocial + socialLevel + equali_genderz + tolerancez + privatez  + respect, data = fulldata_wvs)
gdppc2demscore_sub <- update(gdppc2demscore, demscore_sub ~ .)
gdppc2demscore_directi <- update(gdppc2demscore,demscore_sub ~. - equali_genderz - tolerancez - privatez - respect)

gdppc2equali <- update(gdppc2demscore,equali_genderz ~ . - equali_genderz - tolerancez - privatez - respect)
gdppc2tolerance <- update(gdppc2equali, tolerancez ~ . )
gdppc2private <- update(gdppc2equali, privatez ~ . )
gdppc2respect <- update(gdppc2equali, respect ~ . )
# gdppc2compete <- update(gdppc2equali, competez ~ . )
...
mlist_gdppc2med <- list(
  gdppc2equali = gdppc2equali,
  gdppc2tolerance = gdppc2tolerance,
  gdppc2private = gdppc2private,
  gdppc2respect = gdppc2respect
)
  
list_eco2dem <- list(
  gdppc2demscore_directi = gdppc2demscore_directi,
  gdppc2demscore_sub = gdppc2demscore_sub,
  gdppc2equali = gdppc2equali,
  gdppc2tolerance = gdppc2tolerance,
  gdppc2private = gdppc2private,
  gdppc2respect = gdppc2respect
...
mlist_eco2med <- list(
  Equality = gdppc2equali,
  Tolerance = gdppc2tolerance,
  `Free competition` = gdppc2private,
  `Respect authority` = gdppc2respect
)


msummary(mlist_eco2med,
         stars = TRUE,
         coef_omit = "(Intercept)",
         coef_rename = ls_ctrl,
         output = "../outputs/results_direct.html")
```

### plot_wvs_eco
```{r}
p1_wvs_gdp <- mds_china %>%
  filter(.,year >= "1950" & year < "2002") %>%
  ggplot(.,aes(x = factor(year), y = log(gdppc), group = 1)) +
  geom_line() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +  
  xlab('Year')

p1_wvs_growth <- mds_china %>%
  filter(.,year >= "1950" & year < "2002") %>%
  ggplot(.,aes(x = factor(year), y = growth, group = 1)) +
  geom_line() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +  
  xlab('Year')
```
...
### meidation_sub
```{r acme_media}
med_equality <- mediation::mediate(
  model.m = gdppc2equali,
  model.y = gdppc2demscore_sub,
  treat = "gdppc_mean_log",
  mediator = "equali_genderz",
  sims = 1000,
  robustSE = TRUE,
  dropobs = TRUE)

med_tolerance <- mediation::mediate(
  model.m = gdppc2tolerance,
  model.y = gdppc2demscore_sub,
  treat = "gdppc_mean_log",
  mediator = "tolerancez",
  sims = 1000,
...
med_private <- mediation::mediate(
  model.m = gdppc2private,
  model.y = gdppc2demscore_sub,
  treat = "gdppc_mean_log",
  mediator = "privatez",
  sims = 1000,
...
med_respect <- mediation::mediate(
  model.m = gdppc2respect,
  model.y = gdppc2demscore_sub,
  treat = "gdppc_mean_log",
  mediator = "respect",
  sims = 1000,
...
df_equality <- tibble(
  mediator = med_equality$mediator,
  effect = "aceB",
  estimate = med_equality$d.avg,
  p.value = med_equality$d.avg.p,
  conf.low = med_equality$d.avg.ci[1],
  conf.high = med_equality$d.avg.ci[2],
  prop = scales::percent(med_equality$n.avg)) %>%
  bind_rows(
    tibble(
      mediator = med_equality$mediator,
      effect = "ade",
      estimate = med_equality$z.avg,
      p.value = med_equality$z.avg.p,
      conf.low = med_equality$z.avg.ci[1],
      conf.high = med_equality$z.avg.ci[2],
      prop = "")
      ) %>%
  bind_rows(
    tibble(
      mediator = med_equality$mediator,
      effect = "total",
      estimate = med_equality$tau.coef,
      p.value = med_equality$tau.p,
      conf.low = med_equality$tau.ci[1],
      conf.high = med_equality$tau.ci[2],
      prop = "")
  )

df_tolerance <- tibble(
  mediator = med_tolerance$mediator,
  effect = "aceB",
  estimate = med_tolerance$d.avg,
  p.value = med_tolerance$d.avg.p,
  conf.low = med_tolerance$d.avg.ci[1],
  conf.high = med_tolerance$d.avg.ci[2],
  prop = scales::percent(med_tolerance$n.avg)) %>%
  bind_rows(
    tibble(
      mediator = med_tolerance$mediator,
      effect = "ade",
      estimate = med_tolerance$z.avg,
      p.value = med_tolerance$z.avg.p,
      conf.low = med_tolerance$z.avg.ci[1],
      conf.high = med_tolerance$z.avg.ci[2],
      prop = "")
      ) %>%
  bind_rows(
    tibble(
      mediator = med_tolerance$mediator,
      effect = "total",
      estimate = med_tolerance$tau.coef,
      p.value = med_tolerance$tau.p,
      conf.low = med_tolerance$tau.ci[1],
      conf.high = med_tolerance$tau.ci[2],
      prop = "")
  )

df_private <- tibble(
  mediator = med_private$mediator,
  effect = "aceB",
  estimate = med_private$d.avg,
  p.value = med_private$d.avg.p,
  conf.low = med_private$d.avg.ci[1],
  conf.high = med_private$d.avg.ci[2],
  prop = scales::percent(med_private$n.avg)) %>%
  bind_rows(
    tibble(
      mediator = med_private$mediator,
      effect = "ade",
      estimate = med_private$z.avg,
      p.value = med_private$z.avg.p,
      conf.low = med_private$z.avg.ci[1],
      conf.high = med_private$z.avg.ci[2],
      prop = "")
      ) %>%
  bind_rows(
    tibble(
      mediator = med_private$mediator,
      effect = "total",
      estimate = med_private$tau.coef,
      p.value = med_private$tau.p,
      conf.low = med_private$tau.ci[1],
      conf.high = med_private$tau.ci[2],
      prop = "")
  )

df_respect <- tibble(
  mediator = med_respect$mediator,
  effect = "aceB",
  estimate = med_respect$d.avg,
  p.value = med_respect$d.avg.p,
  conf.low = med_respect$d.avg.ci[1],
  conf.high = med_respect$d.avg.ci[2],
  prop = scales::percent(med_respect$n.avg)) %>%
  bind_rows(
    tibble(
      mediator = med_respect$mediator,
      effect = "ade",
      estimate = med_respect$z.avg,
      p.value = med_respect$z.avg.p,
      conf.low = med_respect$z.avg.ci[1],
      conf.high = med_respect$z.avg.ci[2],
      prop = "")
      ) %>%
  bind_rows(
    tibble(
      mediator = med_respect$mediator,
      effect = "total",
      estimate = med_respect$tau.coef,
      p.value = med_respect$tau.p,
      conf.low = med_respect$tau.ci[1],
      conf.high = med_respect$tau.ci[2],
      prop = "")
  )

mediation_gdppc <- df_equality %>%
  bind_rows(.,df_tolerance,df_private,df_respect)

df_mediation_gdppc <- mediation_gdppc %>%
  filter(effect != "total") %>%
  mutate(
...
df_mediation_gdppc <- df_mediation_gdppc %>%
  group_by(effect) %>%
  mutate(estimate_abs = ifelse(p.value <= 0.05, abs(estimate),0),
         total = sum(estimate_abs),
         propAceB = round(estimate_abs/total,3),
         propAceB = scales::percent(propAceB,accuracy = 0.1),
...
plot_mediation <- DiagrammeR::grViz(diagram = 
       "digraph {
 ...
 e -> g [style = '@@2-4', label = '@@3-4']
  ...
  a -> g [style = 'solid', label = '@@4-2'] 
       }
[1]: df_mediation_gdppc$mediator_lab[df_mediation_gdppc$effect == 'aceB']
[2]: df_mediation_gdppc$lineStyle[df_mediation_gdppc$effect == 'aceB']
[3]: df_mediation_gdppc$arrow[df_mediation_gdppc$effect == 'aceB']
[4]: df_mediation_gdppc$mediator_lab[df_mediation_gdppc$effect == 'ade']
  ")
 # plot_mediation
#   export_svg() %>% 
#   charToRaw() %>% 
...
### table_mediation effect 
```{r}
df_med <- mediation_gdppc %>%
  select(.,-"prop") %>%
...
df_med$se <- (df_med$conf.high - df_med$conf.low) / (2 * qnorm((1 + 0.95) / 2))
...
t2 <- tabulator(
  ...
  bold(part = "header") %>%
  width(j = 1, width = 1) %>% 
  set_caption("Mediative Effect of Econimic on Demcracy Concept") %>%
...
### sensative analysis
```{r}
sens_equality <- mediation::medsens(
  med_equality,
  rho.by = 0.05,
  effect.type = "indirect",
  sims = 10)

sens_tolerance <- mediation::medsens(
  med_tolerance,
  rho.by = 0.05,
  effect.type = "indirect",
  sims = 1)

sens_private <- mediation::medsens(
  med_private,
  rho.by = 0.05,
  effect.type = "indirect",
  sims = 1)

sens_respect <- mediation::medsens(
  med_compete,
  rho.by = 0.05,
  effect.type = "indirect",
  sims = 1)

results_sens_euqality <- list(
  result <- summary(sens_equality),
  key_turn <- which(diff(sign(result$d1)) != 0) + 1,
  rho_turn <- result$rho[key_turn]
)

results_sens_tolerance <- list(
  result <- summary(sens_tolerance),
  key_turn <- which(diff(sign(result$d1)) != 0) + 1,
  rho_turn <- result$rho[key_turn]
)

results_sens_private <- list(
  result <- summary(sens_private),
  key_turn <- which(diff(sign(result$d1)) != 0) + 1,
  rho_turn <- result$rho[key_turn]
)

results_sens_respect <- list(
  result <- summary(sens_respect),
  key_turn <- which(diff(sign(result$d1)) != 0) + 1,
  rho_turn <- result$rho[key_turn]
)

names_sens <- c("Gender quality","Social Tolerance","Free Competition","Respect Authority")

result_cen <- c(results_sens_euqality[[3]],
             results_sens_tolerance[[3]],
             results_sens_private[[3]],
             results_sens_respect[[3]])

results_cen <- data.frame(
  names_sens,
  result_cen
)
results_cen
...
```{r}
plot_direct <- dwplot(
  gdppc2demscore_directi,
  vline = geom_vline(xintercept = 0, colour = "grey60", linetype = 2)) %>%
  relabel_predictors(
    c(
      gdppc_mean_log = "GDP per capita",
      age = "Age",
      edu = "Education",
      socialLevel = "Social Level",
      gender = "Female",
      interstpol = "Political Interst",
      trustsocial = "Social Trust",
      marriage = "Marriage"
    )
  ) + 
  ...
  ylab("") +
  ggtitle("Effects of Econim on Demcracy Concept") +
...
ggsave(plot_direct,file = "../fig/direct_demscore_sub.pdf")
```
...
##### meidation_demscore
```{r acme_media}
med_equality_check <- mediation::mediate(
  model.m = gdppc2equali,
  model.y = gdppc2demscore,
  treat = "gdppc_mean_log",
  mediator = "equali_genderz",
  sims = 1000,
  robustSE = TRUE,
...
med_tolerance_check <- mediation::mediate(
  model.m = gdppc2tolerance,
  model.y = gdppc2demscore,
  treat = "gdppc_mean_log",
  mediator = "tolerancez",
...
med_private_check <- mediation::mediate(
  model.m = gdppc2private,
  model.y = gdppc2demscore,
  treat = "gdppc_mean_log",
  mediator = "privatez",
...
med_respect_check <- mediation::mediate(
  model.m = gdppc2respect,
  model.y = gdppc2demscore,
  treat = "gdppc_mean_log",
  mediator = "respect",
...
df_equality_check <- tibble(
  mediator = med_equality_check$mediator,
  effect = "aceB",
  estimate = med_equality_check$d.avg,
  p.value = med_equality_check$d.avg.p,
  conf.low = med_equality_check$d.avg.ci[1],
  conf.high = med_equality_check$d.avg.ci[2],
  prop = scales::percent(med_equality$n.avg)) %>%
  bind_rows(
    tibble(
      mediator = med_equality_check$mediator,
      effect = "ade",
      estimate = med_equality_check$z.avg,
      p.value = med_equality_check$z.avg.p,
      conf.low = med_equality_check$z.avg.ci[1],
      conf.high = med_equality_check$z.avg.ci[2],
      prop = "")
      ) %>%
  bind_rows(
    tibble(
      mediator = med_equality_check$mediator,
      effect = "total",
      estimate = med_equality_check$tau.coef,
      p.value = med_equality_check$tau.p,
      conf.low = med_equality_check$tau.ci[1],
      conf.high = med_equality_check$tau.ci[2],
      prop = "")
  )

df_tolerance_check <- tibble(
  mediator = med_tolerance_check$mediator,
  effect = "aceB",
  estimate = med_tolerance_check$d.avg,
  p.value = med_tolerance_check$d.avg.p,
  conf.low = med_tolerance_check$d.avg.ci[1],
  conf.high = med_tolerance_check$d.avg.ci[2],
  prop = scales::percent(med_tolerance_check$n.avg)) %>%
  bind_rows(
    tibble(
      mediator = med_tolerance_check$mediator,
      effect = "ade",
      estimate = med_tolerance_check$z.avg,
      p.value = med_tolerance_check$z.avg.p,
      conf.low = med_tolerance_check$z.avg.ci[1],
      conf.high = med_tolerance_check$z.avg.ci[2],
      prop = "")
      ) %>%
  bind_rows(
    tibble(
      mediator = med_tolerance_check$mediator,
      effect = "total",
      estimate = med_tolerance_check$tau.coef,
      p.value = med_tolerance_check$tau.p,
      conf.low = med_tolerance_check$tau.ci[1],
      conf.high = med_tolerance_check$tau.ci[2],
      prop = "")
  )

df_private_check <- tibble(
  mediator = med_private_check$mediator,
  effect = "aceB",
  estimate = med_private_check$d.avg,
  p.value = med_private_check$d.avg.p,
  conf.low = med_private_check$d.avg.ci[1],
  conf.high = med_private_check$d.avg.ci[2],
  prop = scales::percent(med_private_check$n.avg)) %>%
  bind_rows(
    tibble(
      mediator = med_private_check$mediator,
      effect = "ade",
      estimate = med_private_check$z.avg,
      p.value = med_private_check$z.avg.p,
      conf.low = med_private_check$z.avg.ci[1],
      conf.high = med_private_check$z.avg.ci[2],
      prop = "")
      ) %>%
  bind_rows(
    tibble(
      mediator = med_private_check$mediator,
      effect = "total",
      estimate = med_private_check$tau.coef,
      p.value = med_private_check$tau.p,
      conf.low = med_private_check$tau.ci[1],
      conf.high = med_private_check$tau.ci[2],
      prop = "")
  )

df_respect_check <- tibble(
  mediator = med_respect_check$mediator,
  effect = "aceB",
  estimate = med_respect_check$d.avg,
  p.value = med_respect_check$d.avg.p,
  conf.low = med_respect_check$d.avg.ci[1],
  conf.high = med_respect_check$d.avg.ci[2],
  prop = scales::percent(med_respect_check$n.avg)) %>%
  bind_rows(
    tibble(
      mediator = med_respect_check$mediator,
      effect = "ade",
      estimate = med_respect_check$z.avg,
      p.value = med_respect_check$z.avg.p,
      conf.low = med_respect_check$z.avg.ci[1],
      conf.high = med_respect_check$z.avg.ci[2],
      prop = "")
      ) %>%
  bind_rows(
    tibble(
      mediator = med_respect_check$mediator,
      effect = "total",
      estimate = med_respect_check$tau.coef,
      p.value = med_respect_check$tau.p,
      conf.low = med_respect_check$tau.ci[1],
      conf.high = med_respect_check$tau.ci[2],
      prop = "")
  )

mediation_gdppc_check <- df_equality_check %>%
  bind_rows(.,df_tolerance_check,df_private_check,df_respect_check)

df_mediation_gdppc_check <- mediation_gdppc_check %>%
  filter(effect != "total") %>%
  mutate(
    across(where(is.numeric),~ round(.,5)),
    sig = p.value <= 0.05,
...
df_mediation_gdppc_check <- df_mediation_gdppc_check %>%
  group_by(effect) %>%
  mutate(estimate_abs = ifelse(p.value <= 0.05, abs(estimate),0),
         total = sum(estimate_abs),
         propAceB = round(estimate_abs/total,3),
...
plot_mediation_check <- DiagrammeR::grViz(diagram = 
       "digraph {
 graph [layout = dot, rankdir = LR]
 node [shape = box]
 edge [arrowhead = vee]
...
[1]: df_mediation_gdppc_check$mediator_lab[df_mediation_gdppc_check$effect == 'aceB']
[2]: df_mediation_gdppc_check$lineStyle[df_mediation_gdppc_check$effect == 'aceB']
[3]: df_mediation_gdppc_check$arrow[df_mediation_gdppc_check$effect == 'aceB']
[4]: df_mediation_gdppc_check$mediator_lab[df_mediation_gdppc_check$effect == 'ade']
  ")
 # plot_mediation
#   export_svg() %>% 
#   charToRaw() %>% 
...
plot_mediation_check
```

```{r}
gdppc2demscore_direct_check <- update(gdppc2demscore_directi, lag(demscore_sub) ~ . + gdppc_mean_log)


msummary(gdppc2demscore_sub,
         stars = TRUE,
         coef_omit = "(Intercept)",
         coef_rename = ls_ctrl,
         output = "../outputs/results_check.html")
```
...
##### fator analysis
```{r}
df_demscore <- wvs_china %>%
  select(.,E224s_na,E226_na,E227s_na,E229_na)
  
psych::KMO(df_demscore)
bartlett.test(df_demscore)
pc_demscore <- principal(df_demscore,2)
pc_demscore
fa.diagram(pc_demscore$loadings, digits = 3)
