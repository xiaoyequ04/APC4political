...
cbs$ecocountry <- scales::rescale(cbs$ecocountry,to = c(0,1))
cbs$ecocountry5 <- scales::rescale(cbs$ecocountry5,to = c(0,1))
cbs$ecofamily <- scales::rescale(cbs$ecofamily,to = c(0,1))
cbs$ecofamily5 <- scales::rescale(cbs$ecofamily5,to = c(0,1))
...
gdppc2autho_mean <- lm(autho ~ log(gdppc_mean)  + socialLevel +female + edu + hukouRural + interstpol + frepol + marriage + party, data = fulldata_cbs,weights = weight)
...
gdp2ecofamily <- update(gdp2autho,ecofamily ~ .)
