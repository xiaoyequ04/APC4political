...
wvs_china <- WVS_TimeSeries_1981_2022_spss_v3_0 %>%
  filter(.,COW_NUM == 710) %>%
  select(.,S002VS,S020,COW_NUM,COW_ALPHA,E069_11,E069_12,E069_08,E069_07,E114,E115,E116,E117,E114,E115,E116,E117,E117B,E224,E225,E226,E227,E228,E229,E235,E236,E036,E037,E035,X001,X002,X003,X025,X025R,X045,X047_WVS,X047R_WVS,X007,S017,E023) %>%
  dplyr::rename(
    wave = S002VS,
    period = S020,
...
#| eval: false
#| echo: false
t1 <- modelsummary::datasummary(marriage + class_sub + class_fam + interstpol + trust_gover + trust_gover_max +  trust_party + trust_party_max + trust_civil + trust_parli + dempro + demuti + demimportant + demlevel + demperform
...
21. E035: 收入应该尽可能均等 vs 应该加大收入差距以鼓励个人工作
22. E036：私营经济成分应该扩大 vs 国有经济成分应该扩大
23. E037: 国家应该承担更多的责任保障每个人的生活 vs 个人应该承担更多的责任来养活自己
...
cbs2002 <- cbs2002_raw %>%
  dplyr::select(.,q1,province_id,year,wave,v2,v4,v14,v23,v24,v82,v83,v84,v111,v110,v109,v101,v123,v73a,v73b,v73c,v73d,i3c,i4,i5a,i5b,i5c,i5d,i8)  %>%
  rename(id = q1,
...
cbs2002$interstpol[cbs2002$interstpol < 0] <- NA
...
cbs2002$demsatisify[cbs2002$demsatisify < 0] <- NA
cbs2002$i5a <- Recode(cbs2002$i5a,"8:9 = NA")
cbs2002$demelect <- 5 - cbs2002$i5a
cbs2002$i5b <- Recode(cbs2002$i5b,"8:9 = NA")
cbs2002$expert <- 5 - cbs2002$i5b
cbs2002$i5c <- Recode(cbs2002$i5c,"8:9 = NA")
cbs2002$compete <- 5 - cbs2002$i5c
cbs2002$i5d <- Recode(cbs2002$i5d,"8:9 = NA")
...
cbs2008 <- cbs2008_raw %>%
  dplyr::select(.,number,PROV,year,wave,a1,b1,b6,c9,j1,j2,j3,j3a,j4,j8,j28,j35,f30a,f30b,f30c,f30d,g1n,g1o,h9,h11a,h11d,h12) %>%
  rename(id = number,
         province = PROV)
...
cbs2008$interstpol[cbs2008$interstpol < 0] <- NA
cbs2008$socialLevel <- Recode(cbs2008$j25,"98:99 = NA")
cbs2008$ecoLevel <- 6 - cbs2008$c9
cbs2008$ecoLevel <- Recode(cbs2008$ecoLevel,"-3:-2 = NA")

# political trust
cbs2008$trust_party <- cbs2008$f30d
cbs2008$trust_party_z <- Recode(cbs2008$f30d,"7:9 = NA")
cbs2008$trust_party_z <- rescale(as.numeric(cbs2008$trust_party_z),to = c(0,1))
cbs2008$trust_party_max <- Recode(trust_party,"1:5 = 0; 6 = 1; 7:9 = NA")
cbs2008$trust_partyR <- Recode(trust_party,"1:3 = 0; 4: 6 = 1; 7:9 = NA")
...
cbs2008$demsatisify <- 5 - cbs2002$h9
cbs2008$demsatisify[cbs2008$demsatisify < 0] <- NA
...
cbs2011 <- select(.,id,province,year,wave,A1,A2,A8,A15,B1,B11a,B11b,B13,C17a,C17h,C17b,C17c,F12,F13,F14,F15,F16,F17,F18,F19,SE1,SE2,SE3,SE4,SE21,SE22,SE29)
...
cbs2011$trust_party_z <- rescale(as.numeric(cbs2011$trust_party_z),to = c(0,1))
...
cbs2011$demsatisify [cbs2011$demsatisify < 0] <- NA


cbs2011$

cbs2011$leader <- 5 - cbs2011$F16
cbs2011$leader[cbs2011$leader < 0] <- NA
cbs2011$expert <- 5 - cbs2011$F17
cbs2011$expert[cbs2011$expert < 0] <- NA
cbs2011$compete <- 5 - cbs2011$F18
cbs2011$compete[cbs2011$compete < 0] <- NA
cbs2011$army <- 5 - cbs2011$F19
cbs2011$army[cbs2011$army < 0] <- NA
...
cbs2014 <- cbs2014_raw %>%
  select(.,CASEID,Proid,A1,A2,A3A,A4,B1,B11,B18,B19,C19,F7A,F7B,F9,F10,F11,F12,F22A,F22B,F22C,F22D,SE1,SE1A,SE9,SE17,SE8,SE4,SE3,SE2,E1A,E1B,E1H,year,wave) %>%
  rename(id = CASEID,
         province = Proid)
...
cbs2014$ecolevel[cbs2014$ecolevel < 0] <- NA
...
cbs2014$demsatisify[cbs2014$demsatisify < 0] <- NA
...
cbs2014$leader[cbs2014$leader < 0] <- NA
cbs2014$party <- 5 - cbs2014$F22B
cbs2014$party[cbs2014$party < 0] <- NA
cbs2014$army <- 5 - cbs2014$F22C
cbs2014$army[cbs2014$army < 0] <- NA
cbs2014$expert <- 5 - cbs2014$F22D
cbs2014$expert[cbs2014$expert < 0] <- NA
...
cbs2019 <- cbs2019_raw %>%
  dplyr::select(.,"个案编号",SITEPRO,year,wave,A1,A2,A4A,A5,B1,B9,B19,SE1,SE1A,SE11,SE13,SE22,SE5,E4A,E4B,E4C,E4H,F1,F7A,F7B,F9,F10,F11,F12,F24A,F24B,F24C,F24D) %>%
...
bs2019$trust_patryR <- Recode(cbs2019$E4C,"1:3 = 0; 4:6 = 1")
...
::: {.callout-tip}
1. 目前对**党的信任**尚未编码，除了2008还是2002年没有对党的信任，其他年份都有。
:::
