...
wvs_china$equali_gender <- wvs_china %>%
  select(.,D078_na,D060_na,D057_na,D059_na) %>%
  rowMeans(na.rm = TRUE)
...
df_compete <- tibble(
  mediator = med_compete$mediator,
  effect = "aceB",
  estimate = med_compete$d.avg,
  p.value = med_compete$d.avg.p,
  conf.low = med_compete$d.avg.ci[1],
  conf.high = med_compete$d.avg.ci[2],
  prop = scales::percent(med_compete$n.avg)) %>%
  bind_rows(
    tibble(
      mediator = med_compete$mediator,
      effect = "ade",
      estimate = med_compete$z.avg,
      p.value = med_compete$z.avg.p,
      conf.low = med_compete$z.avg.ci[1],
      conf.high = med_compete$z.avg.ci[2],
      prop = "")
      ) %>%
  bind_rows(
    tibble(
      mediator = med_compete$mediator,
      effect = "total",
      estimate = med_compete$tau.coef,
      p.value = med_compete$tau.p,
      conf.low = med_compete$tau.ci[1],
      conf.high = med_compete$tau.ci[2],
      prop = "")
  )

mediation_gdppc <- df_equality %>%
  bind_rows(.,df_tolerance,df_private,df_compete)
...
### robastcheck
```{r}
fulldata_wvs <- fulldata_wvs %>%
  group_by(cohort) %>%
  mutate(dem = mean(demscore_sub,na.rm = TRUE)) %>%
  ungroup()
